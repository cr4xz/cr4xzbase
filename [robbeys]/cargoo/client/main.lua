...
function ResetCargo()
	...
	TriggerServerEvent('Sheikhaay_ki_nono:resetEvent')
...
function DeliverCargo()
	...
	ESX.TriggerServerCallback('Sheikhaay_ki_nono:sellCargo', function(sold)
			if sold then
...
function PurchaseCargo(scenario)
	...
	ESX.TriggerServerCallback('Sheikhaay_ki_nono:getCopsOnline', function(police)

		police = police 

		if police >= Config.Scenarios[scenario].MinCopsOnline then
			
			ESX.TriggerServerCallback('Sheikhaay_ki_nono:buyCargo', function(bought)
			if bought then
